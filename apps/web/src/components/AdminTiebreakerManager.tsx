...
export function AdminTiebreakerManager({ initial }: { initial: TiebreakerQuestionView[] }) {
  ...
  return (
    <section className="flex flex-col gap-4">
      <h2 className="text-lg font-medium">Tiebreaker questions</h2>
      <p className="text-sm text-zinc-500">
        Used only when 2+ players are tied for the top score at the end of a party game — players type in a
        numeric guess and the closest one wins.
      </p>
      <form onSubmit={handleAdd} className="flex flex-col gap-3">
        <input
          placeholder="Prompt, e.g. How many..."
          value={prompt}
        ...
        />
        <input
          placeholder="Reference answer (number)"
          type="number"
          value={answer}
      ...
      <ul className="flex flex-col gap-2">
        {questions.map((q) => (
          ...
          >
            <div>
              ...
              <p className="text-sm text-zinc-500">Answer: {q.answer}</p>
            </div>
            <button
        ...
        {questions.length === 0 && <p className="text-sm text-zinc-500">No tiebreaker questions yet.</p>}
      </ul>
    </section>
  );
}
